...
public class SchemaManifestBuilder {
	...
	public static void main(String[] args) throws Exception {
		...
		File moduleDir = new File(moduleDirPath).getCanonicalFile();
		...
		File outputFile = new File(
			moduleDir,
			"src/main/resources/META-INF/schema/manifest.json");
	...
	private static String _bundleSymbolicName(File moduleDir) throws IOException {
		...
		for (String line : Files.readAllLines(bndFile.toPath())) {
			if (line.startsWith("Bundle-SymbolicName:")) {
				return line.substring("Bundle-SymbolicName:".length()).trim();
			}
		}
	...
	private static File _findRegistratorFile(File moduleDir) throws IOException {
		...
		return Files.walk(srcDir.toPath())
			.filter(
				p -> p.getFileName().toString().endsWith(
