...
PoshiDependenciesFileLocationCheck | [Bug Prevention](bug_prevention_checks.markdown#bug-prevention-checks) | .function, .jar, .lar, .macro, .testcase, .war or .zip | Checks that dependencies files are located in the correct directory. |
PoshiStylingCheck | [Styling](styling_checks.markdown#styling-checks) | .function, .jar, .lar, .macro, .testcase, .war or .zip | Applies rules to enforce consisteny in code style. |
