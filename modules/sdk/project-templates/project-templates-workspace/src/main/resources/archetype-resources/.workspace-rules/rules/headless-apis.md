...
Key Liferay REST modules, their base URIs, primary resources, and OAuth scopes. All paths resolve relative to `http://localhost:${PORT}`. Dev environments accept Basic auth (`test@liferay.com:learn`); production must use OAuth 2 (see `rules/oauth-scopes.md`).
...
| Get site | GET | `/sites/{siteId}` |
| Create page | POST | `/sites/{siteId}/site-pages` |
| List pages | GET | `/sites/{siteId}/site-pages` |
| Create navigation menu | POST | `/sites/{siteId}/site-navigation-menus` |
| Create display page template | POST | `/sites/{siteId}/display-page-templates` |
| Create master page | POST | `/sites/{siteId}/master-pages` |

**Required flag:** `LPD-35443` (off by default) for public layout API; `LPD-38869` (on by default) for private.
...
| Add validation | POST | `/object-definitions/{id}/object-validations` |

Object entries (after publish): `/o/c/<pluralLabel>/` — GET, POST, PUT, PATCH, DELETE by ID.

**OAuth scope:** `Liferay.Object.everything`
